...
def training_double(obsN, latN, obsN1, latN1, POMDP_params, training_params, rnn, net, datestring_data, datestring_train):
    ...
    Train belief network module
    """
    train_loss_bel = np.zeros([NEpochs_bel * 2])
    ...
    for epoch in range(NEpochs_bel):
        ...
        train_loss_bel[epoch + NEpochs_bel] = loss.item()

        if epoch % 10 == 9:
            print("epoch: %d, loss: %1.3f" % (epoch + NEpochs_bel + 1, loss.item()))
            torch.save({
                'epoch': epoch + NEpochs_bel,
                'belNN_state_dict': rnn.state_dict(),
                'bel_optimizer_state_dict': rnn.state_dict(),
                'bel_loss': loss,
            }, path + '/Results/' + datestring_train + '_belNN' + '_epoch' + str(epoch+1+NEpochs_bel)
                + '_data' + datestring_data)

    print("Additional belief network learning finished!")
    ...
    Train action network module
    """
    train_loss_act = np.zeros([NEpochs_act * 2])
    ...
    for epoch in range(NEpochs_act):
        ...
        train_loss_act[epoch + NEpochs_act] = loss.item()

        if epoch % 10 == 9:
            print("epoch: %d, loss: %1.3f" % (epoch + NEpochs_act + 1, loss.item()))
            torch.save({
                'epoch': epoch + NEpochs_act,
                'actNN_state_dict': net.state_dict(),
                'act_optimizer_state_dict': optimizer_act.state_dict(),
                'act_loss': loss,
            }, path + '/Results/' + datestring_train + '_actNN' + '_epoch' + str(epoch + 1 + NEpochs_act)  + '_data' + datestring_data)

    print("Additional action network learning finished!")
    ...
    print('MSE of belief: %f' % criterion(out_bel_batch.squeeze(), target_batch[:, :, 0:2]).item())
    print('KL divergence of action: %f' % criterion_act(torch.log(out_act_batch), target_actDist_batch).item())
    print('Action accuracy: %f' % (correct_count / count))
